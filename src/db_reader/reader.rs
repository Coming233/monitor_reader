...
impl SQLiteReader {
    ...
    where
        T: Copy,
    {
        // 考虑到数据量不大，直接对VecDeque的数据调用insert方法进行插入数据。
